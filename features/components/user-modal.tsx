...
type userModelResponseType =  User & {
  profile: {
      name: string;
      family: string;
  };
        ...
        <div className="z-[2] w-full h-[199px] flex flex-row gap-x-1 shadow-xl">
          ...
          <div className="w-[60%] h-full ">
            ...
            <div className="w-[80%]    mx-auto h-7 my-1 flex justify-end  ">
                <div onClick={response?.id ? ()=>router.push(`/chat?id=new-chat?id=${response?.id}`) : undefined} className="w-[75%] text-white cursor-pointer text-center text-sm rounded-lg border-b-4 border-beh-gray-dark h-full bg-beh-yellow">
                  <h1>گفت و گو</h1>
                </div>
            </div>
          </div>
...

          {
             response?.sellerProfile?.shopName ?
              ...
              :
                ...
                <>
                  <FreeRequestComponent avatar={response?.avatar} username={response?.profile?.name + ' ' + response?.profile?.family} date={elm?.date.toString()} describe={elm?.describe} id={elm?.id} recive_location={''} title={elm?.name}  />
